...
class CombatEngine(
    ...
    val followData = ArrayList<TurnData>()
    ...
    private fun executeActionQueue(onComplete: () -> Unit) {
        ...
        when (char) {
            ...
            '圈' -> {
                ...
                val randomP = getRandomPointInCircle(
                    p.x,
                    p.y,
                    GameConstants.RND_RADIUS_CLICK * coordinateManager.gameScale
                )
    ...
    private fun updateHudText() {
        val nextAction = actionQueue.peek()
        if (nextAction != null) {
            onHudUpdated("执行: ${nextAction.stepIndex}${nextAction.command}", true)
        } else {
            val nextStepStr = getNextTurnFirstAction()
            onHudUpdated("等待: $nextStepStr", false)
        }
    }
    ...
    private fun startSafeDelay(delayTime: Long, block: () -> Unit) {
