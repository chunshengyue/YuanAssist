...
class StrategyDetailActivity : AppCompatActivity() {
        ...
        SupabaseRepository.getStrategyDetail(
            ...
            onSuccess = { detail ->
                if (!canAccessDetail(detail)) {
                    Toast.makeText(this, "该攻略当前不可见", Toast.LENGTH_SHORT).show()
                    finish()
                    return@getStrategyDetail
                }
                ...
                bindHeaderCard(
                    ...
                    tags = JobStationAssetRepository.fromCommunityListItem(detail).tags
                )
            ...
            onError = { message ->
                Toast.makeText(this, "未找到该攻略: $message", Toast.LENGTH_SHORT).show()
            },
        )
    }
    ...
    private fun syncFavoriteState(detail: strategy_detail) {
        ...
        SupabaseRepository.getFavoriteState(
            ...
            onSuccess = { state ->
                if (currentDetail?.objectId != strategyId) return@getFavoriteState
                favoriteObjectId = state.favoriteObjectId.takeIf { it.isNotBlank() }
                currentDetail?.favoriteCount = state.favoriteCount
    ...
    private fun recordStrategyViewIfNeeded(detail: strategy_detail) {
        ...
        SupabaseRepository.incrementStrategyView(strategyId) {
